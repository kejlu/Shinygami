...
#   })
  ...
  output$plot2 <- renderPlotly({
    p_prod_rev <- ggplot(data = prod_rev2, aes(x = Y_Q, y = Rev, color = Product, group = Product )) +
      geom_point(size = 4) + geom_line() +theme(axis.text.x=element_blank()) +
  ...
  output$plot4 <- renderPlotly({
    b_region_rev <- ggplot(data = countries_rev2, aes(x = Y_Q, y = Rev)) + #geom_point(size = 4) +
      xlab('Quarters') + ylab('Revenue (USD in millions)') + ggtitle('Apple Worldwide Revenue 2010 - 2016') +
      geom_bar(aes(fill = countries_rev2$Region), stat = 'identity', position = 'dodge') + theme(axis.text.x=element_blank())
  ...
  output$plot7 <- renderPlot({
    ggplot_waterfall(
      ...
      'Y_Q', 'Americas') + xlab('Quarters') + ylab('Revenue (USD in millions)') + theme(axis.text.x=element_blank()) + ggtitle('Americas')
